...
class ProductosController extends Controller
{
    ...
    public function newAction(Request $request)
    {
        ...
        $em = $this->getDoctrine()->getManager();
        $search = $em->getRepository('CoreBundle:Productos')->findBy(array('sku'=>$producto->getSku()));

        if ($form->isSubmitted() && $form->isValid() && count($search)==0) {
            $em = $this->getDoctrine()->getManager();
    ...
    public function editAction(Request $request, Productos $producto)
    {
        ...
        if ($editForm->isSubmitted() && $editForm->isValid() && count($search)<=1) {
            $em = $this->getDoctrine()->getManager();
            if($producto->getImagen()){
                $file = $producto->getImagen();
